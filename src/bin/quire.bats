...
in let
    (* Library toolbar — always visible *)
    var tb_c = @[char][4]('q', 'l', 't', 'b')
    val tb_id = $W.Generated($S.text_of_chars(tb_c, 4), 4)
    val tb = $W.Element($W.ElementNode(tb_id, $W.Normal($W.Div()), cls_lib_toolbar(), 0, $W.NoneInt(), $W.NoneStr(), $W.WNil()))
    val @(ll, diff) = $W.add_child(ll, tb)
    val () = $D.apply(doc, diff)

    (* Import button — always in toolbar *)
    var ib_c = @[char][4]('q', 'i', 'b', 'n')
    val ib_id = $W.Generated($S.text_of_chars(ib_c, 4), 4)
    val ib = $W.Element($W.ElementNode(ib_id, $W.Normal($W.Div()), cls_import_btn(), 0, $W.NoneInt(), $W.NoneStr(), $W.WNil()))
    ...
    (* Empty library message — hidden when books are present *)
    var el_c = @[char][4]('q', 'e', 'l', 'b')
    val el_id = $W.Generated($S.text_of_chars(el_c, 4), 4)
    val el = $W.Element($W.ElementNode(el_id, $W.Normal($W.Div()), cls_empty_lib(), 0, $W.NoneInt(), $W.NoneStr(), $W.WNil()))
      ...
      lam(_payload_len: int): int => let
        ...
        val p2 = $P.and_then<int><int>(p, lam(result) =>
          if result = 0 then let
            (* Import succeeded — book card was already added by _add_book_card *)
    ...
    var kd_c = @[char][7]('k', 'e', 'y', 'd', 'o', 'w', 'n')
    val kd_arr = $S.from_char_array(kd_c, 7)
    ...
    var sg_c = @[char][4]('q', 's', 'e', 't')
    val sg_narr = $S.from_char_array(sg_c, 4)
    val @(sg_nf, sg_nb) = $A.freeze<byte>(sg_narr)
    var ck8_c = @[char][5]('c', 'l', 'i', 'c', 'k')
    val ck8_arr = $S.from_char_array(ck8_c, 5)
    ...
    var am_c = @[char][4]('q', 'f', 's', 'm')
    val am_narr = $S.from_char_array(am_c, 4)
    val @(am_nf, am_nb) = $A.freeze<byte>(am_narr)
    var ck9_c = @[char][5]('c', 'l', 'i', 'c', 'k')
    val ck9_arr = $S.from_char_array(ck9_c, 5)
    ...
    var ap_c = @[char][4]('q', 'f', 's', 'p')
    val ap_narr = $S.from_char_array(ap_c, 4)
    val @(ap_nf, ap_nb) = $A.freeze<byte>(ap_narr)
    var ck10_c = @[char][5]('c', 'l', 'i', 'c', 'k')
    val ck10_arr = $S.from_char_array(ck10_c, 5)
    ...
    var sc_c = @[char][4]('q', 's', 'c', 'l')
    val sc_narr = $S.from_char_array(sc_c, 4)
    val @(sc_nf, sc_nb) = $A.freeze<byte>(sc_narr)
    var ck11_c = @[char][5]('c', 'l', 'i', 'c', 'k')
    val ck11_arr = $S.from_char_array(ck11_c, 5)
