(* epub_xml -- EPUB XML/OPF parsing and byte-copy helpers *)
...
implement xml_name_eq(data, len, name_off, name_len, pat, plen) =
  _xml_name_eq(data, len, name_off, name_len, pat, plen)

#pub fun walk_rootfile_nodes
  {lb:agz}{n:pos}{sz:nat}
  (data: !$A.borrow(byte, lb, n), len: int n,
   nodes: !$X.xml_node_list(sz)): @(int, int)

implement walk_rootfile_nodes(data, len, nodes) =
  _walk_rootfile_nodes(data, len, nodes)

#pub fun walk_opf_metadata
  {lb:agz}{n:pos}{sz:nat}
  (data: !$A.borrow(byte, lb, n), len: int n,
   nodes: !$X.xml_node_list(sz),
   t_off: int, t_len: int,
...
implement walk_opf_metadata(data, len, nodes, t_off, t_len, a_off, a_len) =
  _walk_opf_metadata(data, len, nodes, t_off, t_len, a_off, a_len)

#pub fun count_spine_items
  {lb:agz}{n:pos}{sz:nat}
  (data: !$A.borrow(byte, lb, n), len: int n,
   nodes: !$X.xml_node_list(sz),
   acc: int): int

implement count_spine_items(data, len, nodes, acc) =
  _count_spine_items(data, len, nodes, acc)

#pub fun find_chapter_href_n
  {lb:agz}{n:pos}{sz:nat}
  (data: !$A.borrow(byte, lb, n), len: int n,
   nodes: !$X.xml_node_list(sz),
   chapter_idx: int): @(int, int)

implement find_chapter_href_n(data, len, nodes, chapter_idx) =
  _find_chapter_href_n(data, len, nodes, chapter_idx)

#pub fun arr_to_text
  {l:agz}{n:pos}
  (src: !$A.arr(byte, l, n), len: int n): $A.text(n)

implement arr_to_text(src, len) = _arr_to_text(src, len)
...
implement copy_from_borrow(src, src_off, src_max, dst, dst_off, dst_max, count) =
  _copy_from_borrow(src, src_off, src_max, dst, dst_off, dst_max, count)

#pub fun copy_arr_region
  {ls:agz}{ns:pos}{ld:agz}{nd:pos}
  (src: $A.arr(byte, ls, ns), src_off: int, src_max: int ns,
   dst: !$A.arr(byte, ld, nd), dst_max: int nd,
   count: int): $A.arr(byte, ls, ns)

implement copy_arr_region(src, src_off, src_max, dst, dst_max, count) =
  _copy_arr_region(src, src_off, src_max, dst, dst_max, count)

#pub fun find_attr_val
  {lb:agz}{n:pos}{sa:nat}{np:pos}
  (data: !$A.borrow(byte, lb, n), len: int n,
   attrs: !$X.xml_attr_list(sa),
   aname: &(@[char][np]), alen: int np): @(int, int)

implement find_attr_val(data, len, attrs, aname, alen) =
  _find_attr_val(data, len, attrs, aname, alen)
...
implement borrow_region_eq(data, len, off_a, off_b, count) =
  _borrow_region_eq(data, len, off_a, off_b, count)
